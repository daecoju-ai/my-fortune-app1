...
# 0) 고정값/버전
# =========================================================
APP_VERSION = "v2026.0027_BONUSLINKFIX"
...
def _bonus_pending_ui(pending_type: str, out_url: str):
    ...
    st.markdown(f"[외부 페이지로 이동]({out_url})")started_at = st.session_state.get("mg_pending_started_at") or _time.time()
...
def render_minigame_screen(dbs):
    ...
    if last is not None:
        ...
        with st.form("mg_entry_form", clear_on_submit=False):
            ...
            submit_label = "응모하기 (성공 정산 후 문자로 쿠폰 발송/실패 추첨)"
