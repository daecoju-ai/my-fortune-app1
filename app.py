...
# 0) 고정값/버전
# =========================================================
APP_VERSION = "v2026.0016_MINIGAME_CLEAN"
...
def _try_minigame_autosubmit(birth: date, last_sec_str: str, reason: str) -> tuple[bool, str]:
    ...
    if last is not None and last_ok is False:
        ...
        with c1:
            if st.button("친구에게 공유하고 +1", use_container_width=True, key="mg_share_bonus"):
...
def mini_game_ui(birth: date, mbti: str, zodiac_ko: str):
    ...
    st.markdown(
        f"<div style='font-size:42px; font-weight:800; letter-spacing:1px; text-align:center; padding:8px 0;'>{_fmt_sec(now_sec)}<span style='font-size:18px; font-weight:700;'> s</span></div>",
        unsafe_allow_html=True,
    )
    st.caption(f"남은 기회: **{attempts}회**")
    ...
    if last is not None and last_ok is False:
        st.markdown("#### 🔁 재도전 기회 얻기")
        st.write("아래 버튼 중 하나를 눌러 **기회를 +1회** 늘릴 수 있어요. (자기확인 방식)")
        st.caption("TIP: 아래의 **링크 복사/공유하기**로 실제 공유 후, **공유 완료 +1**을 눌러주세요.")
    ...
    st.markdown("#### ☕ 커피쿠폰 응모(기록 저장)")
    st.caption("생년월일/MBTI는 자동 반영됩니다. 이름/전화번호와 동의 체크 후 제출하세요.")

    with st.form("minigame_entry_form", clear_on_submit=False):
        col1, col2 = st.columns(2)
        with col1:
            entry_name = st.text_input("이름", value=(st.session_state.get("minigame_profile_name") or st.session_state.get("name") or ""))
        with col2:
            entry_phone = st.text_input("전화번호", value=(st.session_state.get("minigame_profile_phone") or st.session_state.get("phone") or ""))

        st.text_input("생년월일", value=str(birth), disabled=True)
        st.text_input("MBTI", value=(mbti or ""), disabled=True)
        st.text_input("띠", value=(zodiac_ko or ""), disabled=True)
